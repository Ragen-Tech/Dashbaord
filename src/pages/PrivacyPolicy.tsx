...
const PrivacyPolicy = () => {
  const definitions = [
    ...
    { term: 'Cookies', description: 'are small files that are placed on Your computer, mobile device or any other device by a website, containing the details of Your Browse history on that website among its many uses.' },
  ...
  const sections = [
      ...
      content: [
        {
          ...
          paragraphs: [
            ...
            },
            {
              ...
              afterList: 'If You decide to register through or otherwise grant us access to a Third-Party Social Media Service, We may collect Personal data that is already associated with Your Third-Party Social Media Service\'s account, such as Your name, Your email address, Your activities or Your contact list associated with that account. If You choose to provide such information and Personal Data, during registration or otherwise, You are giving the Company permission to use, share, and store it in a manner consistent with this Privacy Policy.'
            },
            {
              ...
              afterList: 'Cookies can be "Persistent" or "Session" Cookies. Persistent Cookies remain on Your personal computer or mobile device when You go offline, while Session Cookies are deleted as soon as You close Your web browser. You can learn more about cookies on [TermsFeed website article](https://www.termsfeed.com/blog/cookies/).',
              subSections: [
                {
                  heading: 'We use both Session and Persistent Cookies for the purposes set out below:',
                  list: [
                    {
              ...
              finalText: 'For more information about the cookies we use and your choices regarding cookies, please visit our Cookies Policy or the Cookies section of our Privacy Policy.'
            }
          ]
        },
        {
    ...
    },
    {
      ...
      icon: Shield, // Re-using Shield icon for related security/legal topics
      content: [
        {
    ...
    },
    {
      ...
      icon: Database, // Re-using Database icon
      content: [
        {
          paragraphs: [
            {
    ...
    },
    {
      ...
      icon: Shield, // Re-using Shield icon
      content: [
        {
          paragraphs: [
            {
              ...
              <ul className="space-y-3">
                {definitions.map((def, index) => (
                  <li key={index} className="flex items-start space-x-3">
                    ...
                    <span className="text-gray-300 leading-relaxed text-justify"><strong>{def.term}</strong> {def.description}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          {sections.map((section, index) => (
            ...
            >
              <div className="bg-gray-900/80 backdrop-blur-sm p-8 rounded-xl border border-gray-800">
                ...
                {section.content.map((mainContent, mainIndex) => (
                  <div key={mainIndex} className="mb-6">
                    {mainContent.subTitle && <h3 className="text-xl font-bold text-white mb-4">{mainContent.subTitle}</h3>}
                    {mainContent.paragraphs.map((paragraph, paraIndex) => (
                      <div key={paraIndex} className="mb-4">
                        {paragraph.heading && <h4 className="text-lg font-semibold text-white mb-2">{paragraph.heading}</h4>}
                        {paragraph.text && <p className="text-gray-300 leading-relaxed text-justify">{paragraph.text}</p>}
                        {paragraph.list && (
                          <ul className="space-y-2 ml-4 mt-2">
                            {paragraph.list.map((item, itemIndex) => (
                              <li key={itemIndex} className="flex items-start space-x-3">
                                <div className="w-2 h-2 bg-green-400 rounded-full mt-2 flex-shrink-0"></div>
                                <span className="text-gray-300 leading-relaxed text-justify" dangerouslySetInnerHTML={{ __html: item }}></span>
                              </li>
                            ))}
                          </ul>
                        )}
                        {paragraph.afterList && <p className="text-gray-300 leading-relaxed mt-2 text-justify" dangerouslySetInnerHTML={{ __html: paragraph.afterList }}></p>}
                        {paragraph.subSections && paragraph.subSections.map((subSection, subIndex) => (
                          <div key={subIndex} className="mt-4">
                            <h5 className="text-md font-semibold text-white mb-2">{subSection.heading}</h5>
                            <ul className="space-y-3 ml-4">
                              {subSection.list.map((cookieItem, cookieIndex) => (
                                <li key={cookieIndex} className="mb-3">
                                  <strong className="text-white">{cookieItem.title}</strong> ({cookieItem.type}) <br />
                                  <span className="text-gray-400">Administered by: {cookieItem.administeredBy}</span> <br />
                                  <span className="text-gray-300 text-justify">{cookieItem.purpose}</span>
                                </li>
                              ))}
                            </ul>
                          </div>
                        ))}
                        {paragraph.finalText && <p className="text-gray-300 leading-relaxed mt-4 text-justify">{paragraph.finalText}</p>}
                      </div>
                    ))}
                  </div>
                ))}
